...
@section('title', __('Edit Testimonial — Admin TSI'))
...
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-dark-300 tracking-tight">{{ __('Edit Testimonial') }}</h1>
            <p class="text-dark-100/50 mt-1 font-medium text-sm">{{ $testimonial->trans('client_name') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-dark-100/5 shadow-sm p-6 md:p-8">
        <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}">
            ...
            <div class="space-y-6">
                <div>
                    <x-admin.locale-tabs :model="$testimonial" field="client_name" label="{{ __('Nama Klien') }}" required="true" placeholder="{{ __('Nama Klien') }}" />
                    @error('client_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <x-admin.locale-tabs :model="$testimonial" field="company_name" label="{{ __('Perusahaan') }}" placeholder="{{ __('Perusahaan') }}" />
                ...
                <div>
                    ...
                    <div class="flex gap-1" x-data="{ rating: {{ old('rating', $testimonial->rating) }} }">
                        @for($i = 1; $i <= 5; $i++)
                        ...
                        @endfor
                        <input type="hidden" name="rating" x-model="rating" value="{{ old('rating', $testimonial->rating) }}">
                    </div>
                    @error('rating') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <x-admin.locale-tabs :model="$testimonial" field="content" label="{{ __('Konten Testimonial') }}" type="textarea" :rows="6" required="true" placeholder="{{ __('Tulis testimonial...') }}" />
