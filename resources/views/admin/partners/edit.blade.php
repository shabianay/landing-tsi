...
@section('title', __('Edit Partner — Admin TSI'))
...
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-dark-300 tracking-tight">{{ __('Edit Partner') }}</h1>
            <p class="text-dark-100/50 mt-1 font-medium text-sm">{{ $partner->trans('name') }}</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.partners.update', $partner) }}" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    @if($partner->logo)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $partner->logo) }}" alt="{{ $partner->trans('name') }}" class="h-20 rounded-xl object-cover">
                        </div>
                    @endif
                ...
                <div class="flex items-center gap-6">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $partner->sort_order) }}" min="0" class="w-24 px-4 py-3 bg-dark-50/50 border border-dark-100/10 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-medium placeholder-dark-100/30">
                        @error('sort_order') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="pt-6">
                        <label class="inline-flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $partner->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-dark-100/20 text-primary-500 focus:ring-primary-500/20">
