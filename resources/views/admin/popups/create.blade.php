...
@section('title', __('Tambah Popup — Admin TSI'))
...
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-dark-300 tracking-tight">{{ __('Tambah Popup') }}</h1>
            <p class="text-dark-100/50 mt-1 font-medium text-sm">{{ __('Buat popup marketing untuk halaman utama') }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-dark-100/5 shadow-sm p-6 md:p-8">
        <form method="POST" action="{{ route('admin.popups.store') }}">
            @csrf
            <div class="space-y-6">
                <div>
                    <x-admin.locale-tabs field="title" label="{{ __('Judul') }}" required="true" placeholder="{{ __('Dapatkan Diskon 20%!') }}" />
                    @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <x-admin.locale-tabs field="content" label="{{ __('Konten') }}" type="textarea" :rows="4" required="true" placeholder="{{ __('Isi pesan marketing di sini...') }}" />
                    @error('content') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        <x-admin.locale-tabs field="button_text" label="{{ __('Teks Tombol') }}" placeholder="{{ __('Hubungi Kami') }}" />
                    ...
                    <div>
                        ...
                        <input type="url" name="button_url" id="button_url" value="{{ old('button_url') }}" placeholder="https://" class="w-full px-4 py-3 bg-dark-50/50 border border-dark-100/10 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-medium placeholder-dark-100/30">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="display_delay" id="display_delay" value="{{ old('display_delay', 2) }}" min="0" max="30" class="w-24 px-4 py-3 bg-dark-50/50 border border-dark-100/10 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-medium">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" name="start_at" id="start_at" value="{{ old('start_at') }}" class="w-full px-4 py-3 bg-dark-50/50 border border-dark-100/10 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-medium">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_at" id="end_at" value="{{ old('end_at') }}" class="w-full px-4 py-3 bg-dark-50/50 border border-dark-100/10 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all font-medium">
