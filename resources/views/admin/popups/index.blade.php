...
@section('title', __('Popup Marketing — Admin TSI'))

@section('content')
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-2xl md:text-3xl font-bold text-dark-300 tracking-tight">{{ __('Popup Marketing') }}</h1>
        <p class="text-dark-100/50 mt-1 font-medium">{{ __('Kelola popup yang muncul di halaman utama') }}</p>
    </div>
    <a href="{{ route('admin.popups.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-500 text-white font-semibold rounded-xl shadow-lg shadow-primary-500/25 hover:bg-primary-600 hover:shadow-xl hover:shadow-primary-500/30 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 text-sm">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-dark-50/50 border-b border-dark-100/5">
                    ...
                    <th class="text-left font-semibold text-dark-300 px-5 py-4 hidden sm:table-cell">{{ __('Tombol') }}</th>
                    <th class="text-center font-semibold text-dark-300 px-5 py-4 hidden md:table-cell">{{ __('Delay') }}</th>
                    <th class="text-center font-semibold text-dark-300 px-5 py-4">{{ __('Status') }}</th>
                    <th class="text-left font-semibold text-dark-300 px-5 py-4 hidden lg:table-cell">{{ __('Dibuat') }}</th>
            ...
            <tbody class="divide-y divide-dark-100/5">
                ...
                <tr class="hover:bg-dark-50/30 transition-colors">
                    <td class="px-5 py-4">
                        <div class="font-medium text-dark-300">{{ $popup->trans('title') }}</div>
                        <div class="text-dark-100/40 text-xs mt-0.5 line-clamp-1">{{ strip_tags($popup->trans('content')) }}</div>
                    </td>
                    <td class="px-5 py-4 text-dark-100/60 hidden sm:table-cell">{{ $popup->trans('button_text') ?? '—' }}</td>
                    <td class="px-5 py-4 text-center text-dark-100/60 hidden md:table-cell">{{ $popup->display_delay }}s</td>
                    ...
                    <td class="px-5 py-4 text-dark-100/60 hidden lg:table-cell text-xs">{{ $popup->created_at->format('d M Y H:i') }}</td>
                    <td class="px-5 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.popups.edit', $popup) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-primary-600 bg-primary-50 rounded-lg hover:bg-primary-100 transition-colors">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                {{ __('Edit') }}
                            </a>
                            <form method="POST" action="{{ route('admin.popups.destroy', $popup) }}" onsubmit="return confirm('{{ __('Yakin ingin menghapus popup ini?') }}')" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="6" class="px-5 py-16 text-center">
                        ...
                        <p class="text-dark-100/60 font-medium">{{ __('Belum ada popup.') }}</p>
                        <a href="{{ route('admin.popups.create') }}" class="text-primary-500 font-semibold text-sm hover:underline mt-2 inline-block">{{ __('Tambah popup pertama') }}</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
