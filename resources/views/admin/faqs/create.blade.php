...
@section('title', __('Tambah FAQ — Admin TSI'))
...
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-dark-300 tracking-tight">{{ __('Tambah FAQ') }}</h1>
            <p class="text-dark-100/50 mt-1 font-medium text-sm">{{ __('Buat pertanyaan baru') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-dark-100/5 shadow-sm p-6 md:p-8">
        <form method="POST" action="{{ route('admin.faqs.store') }}">
            ...
            <div class="space-y-6">
                <div>
                    <x-admin.locale-tabs :model="null" field="question" label="{{ __('Pertanyaan') }}" required="true" placeholder="{{ __('Pertanyaan') }}" />
                    @error('question') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <x-admin.locale-tabs :model="null" field="answer" label="{{ __('Jawaban') }}" type="textarea" :rows="6" required="true" placeholder="{{ __('Jawaban') }}" />
